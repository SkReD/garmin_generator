const {
  bezieDistribute,
  sinRandDistribution
} = require("./distributionFunctions.js");

module.exports.getTrackPoints = function getTrackPoints(
  stepsCount,
  startLatitude,
  startLongitude,
  endLatitude,
  ...
  let time = startTime;
  let coordsDistributionFunc = (i, stepsCount) =>
    sinRandDistribution(i, stepsCount, 0.4, distanceDelta);
  let timeDistributionFunc = (i, stepsCount) =>
    sinRandDistribution(i, stepsCount, 0.4, timeDelta);
  let heightDistributionFunc = (i, stepsCount) =>
    bezieDistribute(i, stepsCount, heightDelta, heightPoints);
  ...
  trackpts.forEach((trackPoint, i) => {
    time = Math.min(
      endTime,
      time + timeStep * timeDistributionFunc(i, stepsCount)
    );
